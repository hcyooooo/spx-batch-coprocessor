              ...
              const uint32_t addr[8])
{
    ...
    SPX_PROFILE_FN_END(SPX_PROFILE_FN_PRF_ADDR);
}
                        ...
                        const spx_ctx *ctx)
{
    ...
    memcpy(R, buf, SPX_N);
    SPX_PROFILE_FN_END(SPX_PROFILE_FN_GEN_MESSAGE_RANDOM);
}
...
#define SPX_INBLOCKS (((SPX_N + SPX_PK_BYTES + SPX_SHAX_BLOCK_BYTES - 1) & \
    ...
    *leaf_idx &= (~(uint32_t)0) >> (32 - SPX_LEAF_BITS);
    SPX_PROFILE_FN_END(SPX_PROFILE_FN_HASH_MESSAGE);
}
