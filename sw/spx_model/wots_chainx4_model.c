    ...
    uint32_t num_steps)
{
    ...
    for (uint32_t step = 0; step < num_steps; step++) {
        ...
        const uint32_t hash_step = start_step + step;

        for (size_t lane = 0; lane < SPX_WOTS_CHAINX4_LANES; lane++) {
            set_hash_addr(step_addr[lane], hash_step);
        ...
        memcpy(chain, thash_out, sizeof(chain));
    }
    ...
    uint32_t num_steps)
{
    ...
    for (size_t lane = 0; lane < SPX_WOTS_CHAINX4_LANES; lane++) {
        ...
        for (uint32_t step = 0; step < num_steps; step++) {
            ...
            set_hash_addr(step_addr, start_step + step);
